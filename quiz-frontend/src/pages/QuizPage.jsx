    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-indigo-200">
      <div className="flex w-full max-w-6xl">
        ...
        <div className="bg-indigo-100 rounded-l-xl p-8 flex flex-col items-center justify-center w-1/4">
          ...
          <div className="text-4xl font-extrabold text-indigo-600">{current + 1}</div>
          <div className="text-lg text-indigo-500 mt-2">of {questions.length}</div>
        ...
        <div className="bg-white shadow-lg px-8 py-10 w-2/4">
          <h2 className="text-3xl font-bold text-center mb-8 text-indigo-700">Quiz</h2>
          <div className="mb-8 p-6 rounded-lg bg-indigo-50 shadow">
            <p className="font-semibold text-lg mb-4 text-indigo-800">{q.question_text}</p>
            <div className="flex flex-col gap-3">
              {['A', 'B', 'C', 'D'].map(opt => (
                ...
                >
                  ...
                  />
                  <span className="text-indigo-700">{q[`option_${opt.toLowerCase()}`]}</span>
                </label>
              ))}
            </div>
          </div>
            ...
            {current < questions.length - 1 ? (
              ...
              <button
                ...
                className={`px-6 py-3 rounded-lg bg-green-600 text-white font-semibold hover:bg-green-700 transition 
                  ${(!answers[q.id] || !isAllAnswered) ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                Submit Quiz
              </button>
            )}
